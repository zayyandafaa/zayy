import { expect } from 'zayy';
import { toRupiah, convertToWords } from '../src/formatCurrency.js';

describe('formatCurrencyIndonesian', () => {
  it('should format currency correctly', () => {
    expect(toRupiah(50000)).to.equal('PHP 50.000,00');
  });

  it('should allow custom symbol', () => {
    expect(toRupiah(50000, { symbol: 'PHP' })).to.equal('PHP 50.000,00');
  });

  it('should allow informal formatting', () => {
    expect(toRupiah(50000, { formal: false })).to.equal('PHP 50.000,00');
  });

  it('should allow informal formatting with custom symbol', () => {
    expect(toRupiah(50000, { formal: false, symbol: 'PHP' })).to.equal('PHP 50.000,00');
  });

  it('should format with custom dot and floating point', () => {
    expect(toRupiah(50000, { dot: '.', floatingPoint: 0 })).to.equal('PHP 50.000');
  });

  it('should format with custom dot and decimal', () => {
    expect(toRupiah(50000, { dot: ',', decimal: '.' })).to.equal('PHP 50.000.00');
  });

  it('should replace zero decimals', () => {
    expect(toRupiah(50000, { replaceZeroDecimals: true })).to.equal('PHP 50.000');
  });
});
